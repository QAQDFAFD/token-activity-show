...
afterEach(cleanup)
describe('MenuBarApp',()=>{
 it('shows refresh outcome, refetches, and opens the full client',async()=>{const bridge=api();window.tokenActivityShow=bridge;render(<MenuBarApp/>);expect(await screen.findByText('等待本地活动')).toBeTruthy();fireEvent.click(screen.getByRole('button',{name:'立即刷新'}));const feedback=(await screen.findByText(/没有变更/)).closest('.refresh-feedback');expect(feedback).not.toBeNull();expect(bridge.getToday).toHaveBeenCalledTimes(2);fireEvent.click(screen.getByRole('button',{name:'打开完整活动'}));await waitFor(()=>expect(bridge.openClient).toHaveBeenCalledOnce())})
