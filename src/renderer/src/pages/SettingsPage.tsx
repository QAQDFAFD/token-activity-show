...
export function SettingsPage(): React.JSX.Element {
  ...
  return (
    <div className="page-content settings-page">
      ...
      <section className="panel" aria-labelledby="refresh-heading"><div className="panel-heading"><div><h2 id="refresh-heading">自动刷新</h2><p>自动刷新仅在 Token Show 打开时运行。</p></div></div><label className="field"><span>刷新间隔</span><select aria-label="刷新间隔" value={settings.refreshIntervalMinutes} onChange={(event) => setSettings({ ...settings, refreshIntervalMinutes: Number(event.target.value) })}>{intervals.map((interval) => <option key={interval} value={interval}>{interval === 0 ? '关闭自动刷新' : `每 ${interval} 分钟`}</option>)}</select></label></section>
