...
export function HourlyActivityChart({ activity }: { activity: readonly HourlyActivity[] }): React.JSX.Element {
  ...
  const bars = activity.map((bucket) => {
    ...
    return <div key={bucket.hour} role="button" tabIndex={0} className={`hourly-bar${bucket.totalInteractions === null ? ' hourly-bar--unknown' : ''}`} aria-label={barAriaLabel(bucket, labels, hourText(bucket.hour))} onFocus={() => setActiveHour(bucket.hour)} onBlur={() => setActiveHour(null)} onMouseEnter={() => setActiveHour(bucket.hour)} onMouseLeave={() => setActiveHour(null)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActiveHour(bucket.hour) } }}><div className="hourly-bar-fill">{segments.length > 0 ? segments : <span className="hourly-bar-zero" aria-hidden="true" />}</div><span className="hourly-bar-label" aria-hidden="true">{bucket.hour}</span></div>
  })

  const gridlines = [0.75, 0.5, 0.25].map((fraction) => <i key={fraction} className="hourly-gridline" style={{ bottom: `${fraction * 100}%` }} />)
